...
class SMSService:
    ...
    def __init__(
        self, sign_name: str = "淮安金烨科技", template_code: str = "SMS_324317774"
    ):
        """
    ...
    def _validate_credentials(self) -> None:
        ...
        if not access_key_id:
            raise ValueError(
                "ALIBABA_DYSMS_ACCESS_KEY_ID environment variable is required"
            )
        if not access_key_secret:
            raise ValueError(
                "ALIBABA_DYSMS_ACCESS_KEY_SECRET environment variable is required"
            )
