...
def test_send_otp():
    ...
    phone_number = "18501052017"  # Test phone number
    print(f"\n=== Testing /api/auth/send-otp for {phone_number} ===")
...
def test_verify_otp():
    ...
    print(
        f"\n=== Testing /api/auth/verify-otp for {phone_number} with code {otp_code} ==="
    )
...
def test_invalid_inputs():
    ...
    url_send = f"{BASE_URL}/api/auth/send-otp"
    url_verify = f"{BASE_URL}/api/auth/verify-otp"
    headers = {"Content-Type": "application/json"}
    ...
    test_cases = [
        ...
        {
            "url": url_verify,
            "payload": {"phone_number": "18501052017"},
            "description": "Missing OTP code",
        },
        {
            "url": url_verify,
            "payload": {"phone_number": "18501052017", "otp_code": "wrong"},
    ...
    for test_case in test_cases:
        ...
        try:
            response = requests.post(
                test_case["url"], json=test_case["payload"], headers=headers
            )
    ...
    if test_send_otp(test_phone):
        ...
        if otp_code:
            ...
            if result:
                ...
                print(
                    f"Access Token: {result.get('session', {}).get('access_token', '')[:50]}..."
                )
