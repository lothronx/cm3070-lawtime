...
def is_rate_limited(phone_number: str) -> bool:
    ...
    recent_attempts = [
        attempt_time for attempt_time in phone_attempts 
...
def create_app(test_config=None):
    ...
    def send_otp():
        ...
        try:
            ...
            if not phone_number:
                return jsonify({"status": "error", "message": "phone_number is required"}), 400
            
            # Validate phone number format
            phone_number = phone_number.strip()
            if not phone_number:
                return jsonify({"status": "error", "message": "phone_number cannot be empty"}), 400
