...
describe('clientService', () => {
  ...
  describe('findClientByName', () => {
    it('should find existing client by name (case-insensitive)', async () => {
      const searchName = 'acme corporation';
      const foundClient = mockClients[0];
      ...
      const result = await clientService.findClientByName(searchName);

      expect(result).toEqual(foundClient);
      expect(supabase.from).toHaveBeenCalledWith('clients');
      expect(mockSelect).toHaveBeenCalledWith('*');
      expect(mockIlike).toHaveBeenCalledWith('client_name', searchName);
    ...
    it('should return null when client not found', async () => {
      ...
      const result = await clientService.findClientByName('Nonexistent Client');

      expect(result).toBeNull();
    });

    it('should return null for empty/null client name', async () => {
      const result1 = await clientService.findClientByName('');
      const result2 = await clientService.findClientByName('   ');
      const result3 = await clientService.findClientByName(null as any);
      const result4 = await clientService.findClientByName(undefined as any);
    ...
    it('should throw error when database query fails', async () => {
      ...
      await expect(clientService.findClientByName('Test Client')).rejects.toThrow('Failed to find client: Database error');
    });
  });
