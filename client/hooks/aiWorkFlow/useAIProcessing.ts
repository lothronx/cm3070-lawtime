...
export function useAIProcessing({ onSuccess, onError }: UseAIProcessingParams = {}) {
  ...
  const { setTasks, setProcessing, setPendingMessage, reset } = useAIWorkflowStore();
  ...
  const processFiles = useCallback(async (
    ...
    try {
      ...
      if (response.success && response.proposed_tasks?.length > 0) {
        ...
        const taskWord = response.proposed_tasks.length === 1 ? 'task' : 'tasks';
        setPendingMessage({
          text: `Found ${response.proposed_tasks.length} ${taskWord} to review`,
          type: 'success'
        });
        onSuccess?.(response.proposed_tasks.length);
      } else {
        // No tasks extracted - treat as empty task list, not an error
        console.log('No tasks extracted - user will manually enter task details');
        setTasks([], sourceType); // Set empty task list but keep source type
        setPendingMessage({
          text: 'No tasks found. Please enter manually.',
          type: 'error'
        });
        onSuccess?.(0); // Success with 0 tasks
      }
    } catch (error) {
      console.error('AI processing failed:', error);
      const errorMessage = error instanceof Error ? error.message : 'AI processing failed';
      setPendingMessage({
        text: 'Task extraction failed. Please use manual input.',
        type: 'error'
      });
