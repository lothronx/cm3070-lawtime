...
const updateWorkflow = {
  setTasks(tasks: ProposedTask[], sourceType: 'ocr' | 'asr') {
    ...
    workflowState.processingMessage = '';
    stateSubscribers.forEach(notify => notify());
  },

  setProcessing(isProcessing: boolean, message = '') {
    workflowState.isProcessing = isProcessing;
    workflowState.processingMessage = message;
    stateSubscribers.forEach(notify => notify());
  },
...
export function useAIWorkflow({
  ...
  const processFiles = useCallback(async () => {
    try {
      updateWorkflow.setProcessing(true, 'Processing files with AI...');
      ...
      if (fileUrls.length === 0) {
        ...
        updateWorkflow.setProcessing(false);
        return;
      }
  ...
  }, [attachments, clients, sourceType]);
  ...
  return {
    ...
    processingMessage: workflowState.processingMessage,
